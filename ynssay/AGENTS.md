# Agent Quality Standards: ynssway

You are an expert Frontend Engineer and Product Designer. Every code change, architectural decision, and copy update must strictly adhere to the high-performance and high-quality standards defined in this manifesto.
...
- [Design](#design)
- [Vercel-Specific Guidelines](#vercel-specific-guidelines)
...
- **Preference order.** CSS > Web Animations API > JavaScript libraries (e.g., Motion).
...
*These preferences reflect Vercel’s brand & product choices.*
- **Copywriting Style.** Active voice. Instead of “The CLI will be installed,” say “Install the CLI.” Be clear & concise. Action-oriented language. Write in second person.
- **Headers & Buttons.** Use Title Case (Chicago style). On marketing pages, use Sentence case. 
- **Placeholders.** Strings: `YOUR_API_TOKEN_HERE`. Numbers: `0123456789`.
- **Numbers.** Use numerals for counts. Instead of “eight deployments” say “8 deployments”.

## Copywriting
- **Active voice.** Instead of “The CLI will be installed,” say “Install the CLI.”
- **Heading & Button Case.** Headings & buttons use Title Case (Chicago). On marketing pages, use sentence case.
- **Clarity & Conciseness.** Be clear & concise. Use as few words as possible.
- **Symbols.** Prefer `&` over `and`.
- **Action-oriented language.** Instead of “You will need the CLI…” say “Install the CLI…”.
- **Consistency.** Keep nouns consistent. Introduce as few unique terms as possible.
- **Tone.** Write in second person. Avoid first person.
- **Consistent Placeholders.** Strings: `YOUR_API_TOKEN_HERE`. Numbers: `0123456789`.
- **Numerals.** Use numerals for counts. Instead of “eight deployments” say “8 deployments”.
- **Currency.** Consistent currency formatting. In any given context, display currency with either 0 or 2 decimal places, never mix both.
- **Spacing.** Separate numbers & units with a space. Instead of 10MB say `10 MB`. Use a non-breaking space `&nbsp;`.
- **Sentiment.** Default to positive language. Frame messages in an encouraging, problem-solving way, even for errors. Instead of “Your deployment failed,” say “Something went wrong—try again or contact support.”
- **Guidance.** Error messages guide the exit. Don’t just state what went wrong—tell the user how to fix it. Instead of “Invalid API key,” say “Your API key is incorrect or expired. Generate a new key in your account settings.” The copy & buttons/links should educate & give a clear action.
- **Precision.** Avoid ambiguity. Labels are clear & specific. Instead of the button label “Continue” say “Save API Key”.